...
async fn build_storage(config: &AppConfig) -> anyhow::Result<Arc<dyn Storage>> {
    ...
    Ok(match db.backend {
        DatabaseBackend::Postgres => {
            Arc::new(thaumiel_storage::PostgresStorage::connect(&db.url, db.max_connections).await?)
        }
...
async fn build_cache(config: &AppConfig) -> anyhow::Result<Arc<dyn Cache>> {
    Ok(match config.cache.backend {
        CacheBackend::Redis => {
            Arc::new(thaumiel_cache::RedisCache::connect(&config.cache.redis_url).await?)
        }
