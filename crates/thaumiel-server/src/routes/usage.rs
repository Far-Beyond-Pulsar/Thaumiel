...
pub async fn summary(State(state): State<AppState>, AdminAuth(identity): AdminAuth) -> ApiResult<Json<UsageSummary>> {
    let page = Pagination { limit: COUNT_CAP, offset: 0 };
    ...
    let validate_calls_last_14_days = usage::validate_history(state.cache.as_ref(), identity.org_id).await;

    Ok(Json(UsageSummary {
        products: products.len() as u32,
        licenses_total: licenses.len() as u32,
        licenses_active: licenses.iter().filter(|l| l.status == LicenseStatus::Active).count() as u32,
