...
impl Storage for PostgresStorage {
    ...
    async fn create_product(&self, product: Product) -> Result<Product> {
        sqlx::query(
            "INSERT INTO products (id, org_id, name, default_keygen_backend, created_at) VALUES ($1, $2, $3, $4, $5)",
        )
    ...
    async fn get_license(&self, id: LicenseId) -> Result<LicenseKey> {
        let row = sqlx::query("SELECT * FROM license_keys WHERE id = $1")
            .bind(uuid_str(id))
            .fetch_optional(&self.pool)
            .await
            .map_err(sqlx_err)?
    ...
    async fn get_license_by_key(&self, key: &str) -> Result<LicenseKey> {
        let row = sqlx::query("SELECT * FROM license_keys WHERE key_value = $1")
            .bind(key)
            .fetch_optional(&self.pool)
            .await
            .map_err(sqlx_err)?
    ...
    async fn set_license_status(&self, id: LicenseId, status: LicenseStatus) -> Result<LicenseKey> {
        ...
        sqlx::query("UPDATE license_keys SET status = $1, revoked_at = COALESCE($2, revoked_at) WHERE id = $3")
    ...
    async fn create_activation(&self, activation: Activation) -> Result<Activation> {
        sqlx::query("INSERT INTO activations (id, license_id, machine_fingerprint, activated_at) VALUES ($1, $2, $3, $4)")
    ...
    async fn count_activations(&self, license_id: LicenseId) -> Result<u32> {
        let row = sqlx::query("SELECT COUNT(*) AS c FROM activations WHERE license_id = $1")
    ...
    async fn create_user(&self, user: User) -> Result<User> {
        sqlx::query(
            "INSERT INTO users (id, org_id, email, password_hash, role, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
        )
    ...
    async fn get_user_by_email(&self, org_id: OrganizationId, email: &str) -> Result<User> {
        let row = sqlx::query("SELECT * FROM users WHERE org_id = $1 AND email = $2")
            .bind(uuid_str(org_id))
            .bind(email)
            .fetch_optional(&self.pool)
            .await
    ...
    async fn append_audit_log(&self, entry: AuditLogEntry) -> Result<AuditLogEntry> {
        sqlx::query(
            "INSERT INTO audit_log (id, org_id, actor, action, target, metadata, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        )
