...
## Storage: one trait, four backends, one row-mapping layer

`Storage` is implemented four times: `PostgresStorage`, `MySqlStorage`, `SqliteStorage` (all in `thaumiel-storage`, each behind its own Cargo feature, all on by default), and `InMemoryStorage`, which backs the test suite and needs no external service at all.

Every SQL-backed implementation uses `sqlx::query`/`query_as` with runtime-bound parameters, not the compile-time-checked `query!` macro. That's a conscious trade: `query!` needs a live database connection (or a maintained offline cache) *at compile time*, and maintaining that for three dialects simultaneously is friction that buys little here, since none of these queries are complex enough for compile-time column-type checking to be pulling much weight. What it does buy is that a schema/mapping bug only needs fixing once, in `thaumiel-storage/src/mapping.rs`, whose row-to-domain-type functions are generic over `R: sqlx::Row` and work unmodified against `PgRow`, `MySqlRow`, and `SqliteRow` alike.
...
- **OIDC / SAML / LDAP auth providers.** The trait is ready; the integrations aren't built. Each is a real project on its own, and building three to production quality wasn't in scope for getting the core system solid.
- **Dynamic (cdylib) or WASM plugin loading.** Considered and deliberately not chosen. Compile-time registration means a plugin author gets full Rust, no ABI stability contract to maintain across compiler versions, and a build failure instead of a runtime crash when something's wrong — at the cost of needing a rebuild to add a plugin, which for the "enterprise extends this in-house" use case this is built for is a reasonable trade, not a compromise made for lack of time.
- **SQL Server.** Would follow the exact same pattern as the three existing SQL backends; just not built yet.
- **Multi-tenant billing/usage metering and an admin web UI.** Both are real products in their own right, layered on top of what exists here rather than baked into it.
