...
`InternalAuthProvider` is the only `AuthProvider` implemented today; it's a plugin like any other (see above), registered under the id `"internal"` and selected via `auth.provider` in config. OIDC, SAML, and LDAP providers would implement the same trait and register the same way — nothing about the route handlers in `routes/auth.rs` assumes there's only ever one provider, it just doesn't yet have a second one shipped, because building three enterprise SSO integrations to production quality was explicitly out of scope for this pass. See [Roadmap](#roadmap).
...
- **SAML auth provider.** The trait is ready; this integration isn't built, and deliberately so for now, not just a time constraint. (LDAP and OIDC *are* built: `thaumiel_auth::LdapAuthProvider` id `"ldap"`, and `thaumiel_auth::OidcAuthProvider` id `"oidc"` reachable via the dedicated `POST /v1/auth/login/oidc` route.) The reason SAML is different: the only Rust SAML crate (`samael`) is pre-1.0 (v0.0.x) and needs `libxml2`/`xmlsec1` as native C library dependencies -- neither available in the environment this project was built in, and neither a one-line install on every platform contributors might be on, which would make SAML the one plugin in this whole workspace that isn't just `cargo build`. More fundamentally, XML signature verification (XML-DSig) is genuinely security-critical code -- a subtle mistake there is a well-documented, real-world SAML vulnerability class -- and implementing it against an immature crate with no real IdP available to test against isn't a risk worth taking just to close out a checklist item. Full reasoning on the tracking issue.
...
Two items that used to be on this list no longer are: an admin web UI (`thaumiel-ui` is built; see its [README](../crates/thaumiel-ui/README.md)) and a SQL Server storage backend (`MssqlStorage`; see the storage section above -- note its "not exercised against a real SQL Server instance" caveat, though, in that section and in its own doc comment).
